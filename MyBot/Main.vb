...
Module Main
    ...
    Async Function MainAsync() As Task
        ' Configure the bot with essential gateway intents (MessageContent is no longer needed for / commands)
        Dim config = New DiscordSocketConfig() With {
            .GatewayIntents = GatewayIntents.Guilds Or
                              GatewayIntents.GuildMembers,
        .AlwaysDownloadUsers = True ' <-- ADD THIS LINE HERE
        }
        ...
        AddHandler _client.Ready, AddressOf ClientReadyAsync
        AddHandler _client.SlashCommandExecuted, AddressOf SlashCommandHandlerAsync

        ' Path to the token file in the application directory
        ' Path to the token file in the configuration subdirectory
        Dim tokenPfad As String = Path.Combine(AppContext.BaseDirectory, "config", "token.txt")
        Dim token As String = ""

        Try
            If File.Exists(tokenPfad) Then
                token = File.ReadAllText(tokenPfad).Trim()
            Else
                Console.WriteLine($"[ERROR] Token file not found at: {tokenPfad}")
                Return
            End If
        ...
        ' Keeps the application running on the Linux VM
        Await Task.Delay(-1)
    End Function

    ' =========================================================================
    ' 1. REGISTRIERUNG DER COMMANDS (Erzeugt die Vorschau beim Tippen)
    ' =========================================================================
    ' =========================================================================
    ' 1. REGISTRIERUNG DER COMMANDS (Asynchron entkoppelt gegen Gateway-Sperren)
    ' =========================================================================
    Private Function ClientReadyAsync() As Task
    ...
    ' =========================================================================
    ' 2. INTERAKTIONS-VERARBEITUNG (Asynchron entkoppelt)
    ' =========================================================================
    Private Function SlashCommandHandlerAsync(command As SocketSlashCommand) As Task
        ' Run inside a background thread to prevent any gateway latency blocking
        Task.Run(Async Function()
                     Try
                         Dim cmdName As String = command.Data.Name.ToLower()

                         ' -----------------------------------------------------------------
                         ' COMMAND: /ping
                         ' -----------------------------------------------------------------
                         If cmdName = "ping" Then
                             Const Version As String = "01.00.00 A"
                             ...
                             ' -----------------------------------------------------------------
                             ' COMMAND: /threadembed (Restricted to "Server Orga")
                             ' -----------------------------------------------------------------
                         ElseIf cmdName = "threadembed" Then
                             ...
                             ' -----------------------------------------------------------------
                             ' COMMAND: /deletethread (Restricted to "Server Orga")
                             ' -----------------------------------------------------------------
                         ElseIf cmdName = "deletethread" Then
                             Dim gUser = TryCast(command.User, SocketGuildUser)
                             If gUser IsNot Nothing AndAlso gUser.Roles.Any(Function(r) r.Name = "Server Orga") Then
                             ...
                             ' -----------------------------------------------------------------
                             ' COMMAND: /movetothread (Restricted to "Server Orga")
                             ' -----------------------------------------------------------------
                         ElseIf cmdName = "movetothread" Then
                             Dim gUser = TryCast(command.User, SocketGuildUser)
                             If gUser IsNot Nothing AndAlso gUser.Roles.Any(Function(r) r.Name = "Server Orga") Then
            ...
            {"{{USER_AVATAR}}", avatarUrl}
                         ...
                         ElseIf cmdName = "roles" Then
                             ...
                             ' -----------------------------------------------------------------
                             ' COMMAND: /news (Post JSON template as Webhook to specific channel)
                             ' -----------------------------------------------------------------
                         ElseIf cmdName = "news" Then
                ...
                {"{{CHANNEL_NAME}}", targetChannel.Name}
                        ...
                        avatarUrl:="https://discordapp.com"
                             ...
                             ' -----------------------------------------------------------------
                             ' COMMAND: /channels (Powered by the Template Engine)
                             ' -----------------------------------------------------------------
                         ElseIf cmdName = "channels" Then
                             Dim guildUser = TryCast(command.User, SocketGuildUser)
    ...
    {"{{CHANNEL_TREE}}", jsonSafeTree} ' <-- Hier den bereinigten String nutzen!
                     ...
                     Catch ex As Exception
                         Console.WriteLine($"Error inside interaction handler: {ex.Message}")
                     End Try
                 End Function)
        Return Task.CompletedTask
    End Function
