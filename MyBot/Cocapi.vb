...
Public Class ChocolateClashAPI
    Public Shared Async Function DestroyFlareSolverrSessionAsync() As Task
        Try
            ...
            End Using
        Catch ex As Exception
            ' Non-critical fail safe logging
            Console.WriteLine($"[SESSIONS] Session cleanup bypassed: {ex.Message}")
        End Try
    End Function
    Private Shared ReadOnly _handler As New HttpClientHandler() With {.AutomaticDecompression = DecompressionMethods.GZip Or DecompressionMethods.Deflate Or DecompressionMethods.Brotli}
    Private Shared ReadOnly _httpClient As New HttpClient(_handler)
    ' Stores the persistent FlareSolverr session identifier
    ...
    Public Shared Async Function EnsureFlareSolverrSessionAsync() As Task
        Try
            ' 1. Fetch all active sessions from FlareSolverr
            ...
            Using requestList As New HttpRequestMessage(HttpMethod.Post, _flaresolverrUrl)
                ...
                Using responseList As HttpResponseMessage = Await _httpClient.SendAsync(requestList)

                    Dim responseString As String = Await responseList.Content.ReadAsStringAsync()
                    Dim jsonDoc As Newtonsoft.Json.Linq.JObject = Newtonsoft.Json.Linq.JObject.Parse(responseString)
                    Dim sessions As Newtonsoft.Json.Linq.JArray = CType(jsonDoc("sessions"), Newtonsoft.Json.Linq.JArray)

                    ' Check if our specific session ID is already present in the active pool
                    If sessions IsNot Nothing AndAlso sessions.Contains(FlareSolverrSessionId) Then
                        API_COC.DebugPrint($"[SESSIONS] Persistent session '{FlareSolverrSessionId}' is already active.")
                        Return
                    End If
                End Using
            End Using

            ' 2. If session does not exist, create it cleanly
            API_COC.DebugPrint($"[SESSIONS] Creating new persistent session: {FlareSolverrSessionId}...")
            ...
            Using requestCreate As New HttpRequestMessage(HttpMethod.Post, _flaresolverrUrl)
                ...
                Using responseCreate As HttpResponseMessage = Await _httpClient.SendAsync(requestCreate)
                    responseCreate.EnsureSuccessStatusCode()
                    API_COC.DebugPrint($"[SESSIONS SUCCESS] Session '{FlareSolverrSessionId}' successfully spawned.")
                End Using
            End Using

        Catch ex As Exception
            Console.WriteLine($"[SESSIONS CRITICAL] Failed to manage persistent background session: {ex.Message}")
        End Try
    End Function
    Public Shared Async Function GetChocolateClanAsync(clanTag As String) As Task
        ...
        Dim localProxyUrl As String = "http://localhost:8191/v1"
        Dim payloadObj As New With {
        .cmd = "request.get",
        .url = targetUrl,
        .maxTimeout = 60000
    }
        ...
        Using request As New HttpRequestMessage(HttpMethod.Post, localProxyUrl)
            request.Content = New StringContent(jsonPayload, Encoding.UTF8, "application/json")

            Try
                API_COC.DebugPrint("[CHOCOLATE] Sende Anfrage an lokalen FlareSolverr...")

                Using response As HttpResponseMessage = Await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead)
                    response.EnsureSuccessStatusCode()

                    Dim proxyResponseString As String = Await response.Content.ReadAsStringAsync()
                    Dim jsonDoc As JObject = JObject.Parse(proxyResponseString)

                    ' FlareSolverr liefert hier das nackte HTML der FWA-Farming-Webseite
                    Dim rawHtmlContent As String = jsonDoc("solution")?("response")?.ToString()

                    If String.IsNullOrEmpty(rawHtmlContent) Then
                        API_COC.DebugPrint("[CHOCOLATE ERROR] FlareSolverr hat kein HTML geliefert.")
                        Return
                    End If

                    ' HTML parsen statt JSON deserialisieren
                    Dim htmlDoc As New HtmlDocument()
                    htmlDoc.LoadHtml(rawHtmlContent)
                    ' 2. Den absoluten Pfad auf der Oracle VM definieren (Erstellt 'clan_cache.html' im Anwendungsverzeichnis)
                    Dim logFilePath As String = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "clan_cache.html")

                    Try
                        ' 3. Den rohen HTML-String asynchron als UTF-8 codierte Datei schreiben
                        Await File.WriteAllTextAsync(logFilePath, rawHtmlContent.Trim(), Encoding.UTF8)
                        API_COC.DebugPrint($"[CHOCOLATE SUCCESS] Rohes HTML wurde lokal auf der VM gespeichert: {logFilePath}")
                    Catch fileEx As Exception
                        API_COC.DebugPrint($"[CHOCOLATE STORAGE ERROR] Schreiben der Datei fehlgeschlagen: {fileEx.Message}")
                    End Try
                    ' BEISPIEL: Suchen nach Tabellenzeilen (<tr>) oder bestimmten Klassen
                    ' Passen Sie das XPath-Muster an die Struktur von cc.fwafarm.com an!
                    Dim memberNodes = htmlDoc.DocumentNode.SelectNodes("//table//tr")

                    'If memberNodes IsNot Null Then
                    'For Each node In memberNodes
                    ' Hier die Zellen <td> auslesen und der Liste hinzufügen
                    ' Dim name As String = node.SelectSingleNode(".//td[1]")?.InnerText
                    ' memberList.Add(New ChocolateMember() With { .Name = name })
                    'Next
                    'End If

                    API_COC.DebugPrint($"[CHOCOLATE SUCCESS]  Mitglieder aus HTML extrahiert.")
                End Using

            Catch ex As Exception
                Console.WriteLine($"[LOCAL BYPASS CRITICAL] Fehler: {ex.Message}")
            End Try
        End Using
    ...
    Public Shared Async Function GetChocolateClanWarAsync(clanTag As String) As Task(Of String)
        ...
        Dim baseDelayMilliseconds As Integer = 2000 ' Start with a 2-second delay
        Dim operationTimeoutSeconds As Integer = 45 ' Global hard limit for FlareSolverr to finish

        ' Make sure the persistent browser tab is warm and active
    ...
    .cmd = "request.get",
    .session = FlareSolverrSessionId, ' Reuses the existing background browser tab
    .url = targetUrl,
    .maxTimeout = 30000
}
        ...
        ' 2. Execution Loop with Auto-Retry
        For attempt As Integer = 1 To maxRetryAttempts
            ' Create a linked CancellationTokenSource to enforce our local hard execution timeout
            Using timeoutCts As New CancellationTokenSource(TimeSpan.FromSeconds(operationTimeoutSeconds))
                Try
                    API_COC.DebugPrint($"[CHOCOLATE] Fetching via session '{FlareSolverrSessionId}' (Attempt {attempt}/{maxRetryAttempts})...")

                    Using request As New HttpRequestMessage(HttpMethod.Post, _flaresolverrUrl)
                        request.Content = New StringContent(jsonPayload, Encoding.UTF8, "application/json")

                        ' Pass the cancellation token straight into the network send operation
                        Using response As HttpResponseMessage = Await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, timeoutCts.Token)
                            ...
                            ' Verify FlareSolverr's top level operation status wrapper
                            ...
                            Dim rawHtmlContent As String = jsonDoc("solution")?("response")?.ToString()
                            If String.IsNullOrEmpty(rawHtmlContent) Then
                                Throw New Exception("Active session context returned an empty web payload string block.")
                            End If
                            ...
                            Dim winnerBoxNode As HtmlNode = htmlDoc.DocumentNode.SelectSingleNode("//p[@class='winner-box']")
                            If winnerBoxNode IsNot Nothing Then
                                Dim fullText As String = winnerBoxNode.InnerText
                                Dim lines As String() = fullText.Split(New String() {Environment.NewLine, vbLf, vbCr}, StringSplitOptions.RemoveEmptyEntries)

                                For Each line As String In lines
                                    Dim trimmedLine As String = line.Trim()
                                    If trimmedLine.Contains("should win") OrElse trimmedLine.Contains("winner") Then
                                        winstring = System.Net.WebUtility.HtmlDecode(trimmedLine)
                                        Exit For
                                    End If
                                Next
                                API_COC.DebugPrint($"[CHOCOLATE SUCCESS] Successfully extracted winstring: {winstring}")
                            End If

                            ' Operation succeeded completely -> Exit the retry loop entirely
                            Console.WriteLine("jj_" & winstring & "_jj")
                ...
                Catch ex As OperationCanceledException
                    API_COC.DebugPrint($"[CHOCOLATE TIMEOUT] Attempt {attempt} exceeded local processing deadline of {operationTimeoutSeconds}s.")
                Catch ex As Exception
                    API_COC.DebugPrint($"[CHOCOLATE WARNING] Attempt {attempt} failed via framework error exception: {ex.Message}")
                End Try
            End Using

            ' Calculate Exponential Backoff Delay: 2s, then 4s, etc. (Skip delay on final attempt execution)
            If attempt < maxRetryAttempts Then
                Dim currentDelay As Integer = baseDelayMilliseconds * CInt(Math.Pow(2, attempt - 1))
                API_COC.DebugPrint($"[CHOCOLATE RETRY] Sleeping for {currentDelay}ms before engaging next target pipeline request...")
                Await Task.Delay(currentDelay)
            End If
        Next

        API_COC.DebugPrint($"[CHOCOLATE CRITICAL] Exhausted all {maxRetryAttempts} processing attempts without reaching data targets.")
        Return winstring
    End Function
